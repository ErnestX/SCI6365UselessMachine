...
namespace UselessMachineLightSourceFinder.UnitTests
{
	...
	public class SensorReadingTests
	{
		...
		public void TestParsingData_OneLineWithMoreReadingsThanExpected_DiscardLine()
		{
			...
			var trainingData = SensorReading.ProcessSensorData(testString, 3);
			Assert.AreEqual(1, trainingData.Count);
			Assert.AreEqual(3, trainingData[0].NumOfSensors);
			Assert.AreEqual(12, trainingData[0].ReadingOfEachSensor[0]);
			Assert.AreEqual(23, trainingData[0].ReadingOfEachSensor[1]);
			Assert.AreEqual(34, trainingData[0].ReadingOfEachSensor[2]);
		}
	}
}
